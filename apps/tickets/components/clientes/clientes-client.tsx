...
import { Plus, Search, Building2, User, Phone, Mail, MapPin, Trash2, Pencil, ChevronRight, Users, Star } from "lucide-react"
            ...
            {selectedCliente.contactos && selectedCliente.contactos.length > 0 && (
              <div className="space-y-2 pt-2 border-t border-slate-200">
                ...
                {selectedCliente.contactos.map((ct) => (
                  <div key={ct.id} className="flex items-start gap-2 bg-slate-50 rounded-md px-2.5 py-2 text-sm">
                    <div className="flex-1 min-w-0">
                      ...
                      {ct.cargo && (
                        <p className="text-xs text-slate-500">{ct.cargo}</p>
                      )}
                      <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5">
                        <span className="text-xs text-slate-600">{ct.telefono}</span>
                        {ct.email && (
