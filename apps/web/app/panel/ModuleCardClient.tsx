...
// Modules that need a fresh bridge token generated at click time (not at page load)
const BRIDGE_MODULES = new Set(["tickets"])

export default function ModuleCardClient({ module }: { module: Module }) {
    ...
    const handleClick = async (e: React.MouseEvent) => {
        ...
        try {
            let destination = module.href;

            // Generate a fresh bridge token at click time so it never expires
            if (BRIDGE_MODULES.has(module.id)) {
                const res = await fetch(`/api/bridge?module=${module.id}`)
                const data = await res.json() as { url?: string; error?: string }
                if (!data.url) {
                    setError(data.error ?? "Error al generar acceso. Intenta de nuevo.")
